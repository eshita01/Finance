...
def build_graph(
    ...
    news_fetcher = NewsSentimentFetcher(
        [ticker], alpha_key, base_date=base_date, cache_dir=cache_dir
    )
    insider_fetcher = InsiderDataFetcher(ticker, finnhub_key, base_date=base_date)
    peer_fetcher = PeerDataFetcher(
        ticker,
        finnhub_key,
        alpha_key,
        base_date=base_date,
        news_cache_dir=str(cache_dir),
    )
    ...
    def analysis_node(state: AgentState) -> AgentState:
        ...
        signals = analyze(df)
        sentiment = analyze_with_cache(ticker, state["news"], base_date, result_dir)
...
def main() -> None:
    ...
    for day in trading_days:
        ...
        decision = run_for_date(
            args.ticker,
            day.to_pydatetime().replace(tzinfo=timezone.utc),
            keys,
            cache_dir,
            result_dir,
        )
